	...
	*/
	 //START YOUR IMPLEMENTATION BELOW THIS LINE
		...
		public static void main(String[] args) {
			Week06TicTacToeLab game = new Week06TicTacToeLab();
			game.run();
		}
		...
		public void run() {
			while (!gameOver) {
				display();
				getUserInput();
				gameOver = gameBoard.hasPlayerWon(player);
				if (!gameOver) {
					gameOver = gameBoard.isTieGame();
				}
		...
		public String changePlayerTurn(String player) {
			if (player == "X") {
				player = "O";
			} else {
				player = "X";
		...
		public void display() {
			...
			System.out.print(gameBoard.displayGameBoard());
		...
		public void displayWinner() {
			...
			System.out.println(gameBoard.getWinnerText());
			System.out.println("*****************************");
		...
		public void getUserInput() {
			...
			while (selection == -1) {
				...
				if (!gameBoard.isValidMove(selection)) {
					selection = -1;
					System.out.println("     Invalid selection");
				}
			}
			System.out.println("     Player " + player + " chose: " + selection);
			gameBoard.set(selection - 1, player);
		}
}